...
make is one a person can change their mind about: it is refused before it starts when the
device lacks what the model's quantisation requires, stopped while it runs, and deleted
afterwards. It returns text only; the consuming product still owns proposal meaning,
